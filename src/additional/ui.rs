...
//impl //cgc::Collectable for MinifbWin {}
        ...
        match key {
            ValueData::String(s) => {
                ...
                match s {
                    ...
                    "is_key_pressed" => {
                        return Some(Property::new("is_key_pressed", new_exfunc(key_pressed)))
                    }
                    "is_key_released" => {
                        return Some(Property::new("is_key_released", new_exfunc(key_pressed)))
                    }
                    "is_open" => return Some(Property::new("is_open", new_exfunc(is_open))),
                    "is_mouse_down" => {
                        return Some(Property::new("is_mouse_down", new_exfunc(is_mouse_down)))
                    }
                    "mouse_pos" => {
                        let pos = self
                            .0
                        ...
                        let obj = new_object();
                        obj.borrow_mut()
                            .set("x", new_ref(ValueData::Number(pos.0 as f64)))
                            .unwrap();
                        obj.borrow_mut()
                            .set("x", new_ref(ValueData::Number(pos.1 as f64)))
                            .unwrap();
...
pub fn minifb_init(env: Environment) {
    pub fn window(_: &mut Frame<'_>, _: Value, args: &[Value]) -> Result<Value, ValueData> {
        let win = Window::new(
            &val_str(&args[0]),
            val_int(&args[1]) as usize,
            val_int(&args[2]) as usize,
        ...
        match win {
            Ok(win) => {
                ...
                let val = new_userdata(Box::new(win));
                //cgc::gc_add_root(val.gc());
                Ok(val)
            }
            Err(e) => {
                return Err(new_error(
                    -1,
                    None,
                    &format!("failed to create minifb window: {}", e.to_string()),
                ))
            }
        }
    }
    ...
    obj.borrow_mut()
        .set("Key", new_ref(ValueData::Object(keys)))
        .unwrap();
    env.borrow_mut()
        .set("egg", new_ref(ValueData::Object(obj)))
        .unwrap();
}
