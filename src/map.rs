...
unsafe fn drop_empty_node<K, V>(the_box: *mut Node<K, V>) {
    // Prevent compiler from trying to drop the un-initialized key and values in
    // the node.
...
impl<K: Hash + Eq, V, S: BuildHasher> LinkedHashMap<K, V, S> {
    ...
    /// insertion. Iterator element type is `(&'a K, &'a V)`
    ///
    /// # Examples
    /// ```
    /// use mu_utils::LinkedHashMap;
    ...
    /// Returns a double-ended iterator visiting all key-value pairs in order of
    /// insertion. Iterator element type is `(&'a K, &'a mut V)`
    /// # Examples
    /// ```
    /// use mu_utils::LinkedHashMap;
    ///
    ...
    /// Returns a double-ended iterator visiting all values in order of
    /// insertion.
    ///
    /// # Examples
    /// ```
...
/// An insertion-order iterator over a `LinkedHashMap`'s entries, with immutable
...
/// An insertion-order iterator over a `LinkedHashMap`'s entries, with mutable
/// references to the values.
