//! This implementation copied from [atomic-refcell](https://github.com/bholley/atomic_refcell)
//! and uses internally AtomicU32 instead of AtomicUsize to save memory space.
//!
...
const HIGH_BIT: u32 = !(::std::u32::MAX >> 1);
const MAX_FAILED_BORROWS: u32 = HIGH_BIT + (HIGH_BIT >> 1);
...
impl<'b> AtomicBorrowRef<'b> {
    #[inline]
    fn new(borrow: &'b AtomicU32) -> Self {
    ...
    #[cold]
    #[inline(never)]
    fn do_panic(borrow: &'b AtomicU32, new: u32) {
        if new == HIGH_BIT {
...
impl<'b> AtomicBorrowRefMut<'b> {
    #[inline]
    fn new(borrow: &'b AtomicU32) -> AtomicBorrowRefMut<'b> {
