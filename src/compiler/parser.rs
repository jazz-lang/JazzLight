...
use super::{ast::*, lexer::*, msg::*, reader::Reader, token::*};
...
impl<'a> Parser<'a> {
    pub fn new(reader: Reader, ast: &'a mut Vec<P<Expr>>) -> Parser<'a> {
        Self {
            ...
            token: Token::new(
                TokenKind::End,
                Position::new(super::P("<>".to_owned()), 1, 1),
            ),
            ast,
        }
    }
    ...
    fn parse_new(&mut self) -> EResult {
        let pos = self.expect_token(TokenKind::New)?.position;
        let call = self.parse_expression()?;
        match &call.decl {
            ExprDecl::Call(expr, args) => {
                return Ok(expr!(ExprDecl::New(expr.clone(), args.clone()), pos));
            }
            _ => {
