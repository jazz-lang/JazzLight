...
pub fn math_random_int(frame: &mut Frame<'_>,_: Value,args: &[Value]) -> Result<Value,ValueData> {
    if args.len() < 2 {
    ...
    Ok(
        new_ref(math_floor(frame,nil(),&[new_ref(val * (max - min.clone()))])?.borrow().clone() + min)
    )
}
...
pub fn math_object() -> Ref<Object> {
    ...
    object.borrow_mut().set("randomInt",new_exfunc(math_random_int));
    object.borrow_mut().set("E",new_ref(ValueData::Number(std::f64::consts::E)));
    object.borrow_mut().set("PI",new_ref(ValueData::Number(std::f64::consts::PI)));
    object
}