...
impl<T: Trace> Gc<T>
{
    ...
    pub fn new(value: T) -> Self
    {
        ...
        unsafe {
            ...
            let gc = Gc { ptr_root: Cell::new(NonNull::new_unchecked(ptr.as_ptr())),
                          marker: PhantomData };
            gc.set_root();
            gc
        }
...
impl<T: Trace + ?Sized> Gc<T>
{
    ...
    pub unsafe fn from_raw(ptr: *const T) -> Self
    {
        ...
        Gc { ptr_root: Cell::new(NonNull::new_unchecked(rc_ptr)),
             marker: PhantomData }
    }
}
...
impl<T: Trace + ?Sized> Clone for Gc<T>
{
    ...
    fn clone(&self) -> Self
    {
        unsafe {
            self.inner().root_inner();
            let gc = Gc { ptr_root: Cell::new(self.ptr_root.get()),
                          marker: PhantomData };
            gc.set_root();
            gc
        }
...
impl<T: Trace> GcCell<T>
{
    ...
    pub fn new(value: T) -> Self
    {
        GcCell { flags: Cell::new(BORROWFLAG_INIT),
...
unsafe impl<T: Trace + ?Sized> Trace for GcCell<T>
{
    ...
    fn finalize_glue(&self)
    {
        ...
        match self.flags.get().borrowed()
        {
            BorrowState::Writing => (),
            _ =>
            unsafe { (*self.cell.get()).finalize_glue() },
        }
    }
}
...
impl<T: Trace + ?Sized + Debug> Debug for GcCell<T>
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result
    {
        match self.flags.get().borrowed()
        {
            BorrowState::Unused | BorrowState::Reading => f.debug_struct("GcCell")
                                                           .field("value", &self.borrow())
                                                           .finish(),
            BorrowState::Writing => f.debug_struct("GcCell")
